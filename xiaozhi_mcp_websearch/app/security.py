...
SECRET_PATTERNS = [
    re.compile(r"(?i)(brave[_-]?api[_-]?key|api[_-]?key|token|password|secret)(['\"\s:=]+)([^,'\"\s}]+)"),
    re.compile(r"(?i)(X-Subscription-Token)(['\"\s:=]+)([^,'\"\s}]+)"),
...
def is_private_ip(hostname_or_ip: str) -> bool:
    ...
    for info in infos:
        resolved = info[4][0]
        try:
            if _ip_is_blocked(ipaddress.ip_address(resolved)):
                return True
        except ValueError:
            return True
    return False
...
def sanitize_log_value(value: Any) -> Any:
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            key_string = str(key)
            if re.search(r"(?i)(key|token|password|secret)", key_string):
                sanitized[key] = "***"
            else:
