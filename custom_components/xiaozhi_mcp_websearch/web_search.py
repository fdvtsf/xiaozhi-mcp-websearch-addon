...
async def _bocha_web_search(
    ...
    raw_results = _extract_bocha_web_items(data)
...
def _extract_bocha_web_items(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    roots = (payload, data)
    for root in roots:
        for key in ("webPages", "webpages", "web_pages"):
            pages = _as_dict(root.get(key))
            value = pages.get("value") or pages.get("results")
...
async def _post_json(
    ...
    _log_search_call(tool_name, query, count, _count_search_results(data), started)
    return data


def _count_search_results(data: dict[str, Any]) -> int:
    bocha_count = len(_extract_bocha_web_items(data))
    if bocha_count:
        return bocha_count
